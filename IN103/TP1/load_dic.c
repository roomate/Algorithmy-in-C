...
struct word_list_t {
    char *word ; /* A word */
    struct word_list_t *next ; /*A pointer towards the next word */
  };



char **load_file (char *fname, unsigned int *size)
    /*This script takes a file with argument */
    {
    char **words_array ;
    ...
    if (new_cell == NULL) {
        ...
        /* BAD: We should also free the already allocated string list cells, but
            I'm too lazy ^^ Don't do this at home... */
        return (NULL) ;
    }
    ...
    /* Now, link the cell. */
    new_cell->next = wl ; /*pointer towards the next struct word_list_cell*/
    wl = new_cell ; 
    count_words++ ;
